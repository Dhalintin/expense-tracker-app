    ...
    static register(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const token = jwt_util_1.tokenService.generateToken(user.id);
                const userResponse = { id: user.id, name: user.name, email: user.email };
                new response_util_1.default(400, false, "User registered successfully", res, userResponse);
                return;
            }
            catch (err) {
    ...
    static login(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const passwordMatch = yield (0, hash_util_1.comparePassword)(password, existingUser.password);
                if (!passwordMatch) {
                    new response_util_1.default(401, false, "Email or Password incorrect!", res);
                    return;
                }
                const token = jwt_util_1.tokenService.generateToken(existingUser.id);
                const userResponse = {
                    token,
                    id: existingUser.id,
